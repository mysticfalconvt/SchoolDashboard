...
const MY_CALLBACK_ASSIGNMENTS = gql`
  query MY_CALLBACK_ASSIGNMENTS($me: ID) {
    ...
    messagesCount(
      where: { receiver: { id: { equals: $me } }, read: { equals: false } }
    )
  }
`;
...
const MessagesCount: React.FC<MessagesCountProps> = ({ mobile = false }) => {
  const me = useUser();
          ...
          <>
            <button
              ...
              style={
                mobile
                  ? {}
                  : { minWidth: '2.5rem', width: '2.5rem', padding: 0 }
              }
            ...
            >
              <span
                className={
                  mobile
                    ? 'skew-x-[20deg] w-full text-center text-lg'
                    : 'skew-x-[20deg] w-full text-center text-lg'
                }
              >
