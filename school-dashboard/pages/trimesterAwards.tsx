...
interface Student {
  id: string;
  name: string;
  awards?: TrimesterAward[];
}

interface TrimesterAward {
  id: string;
  teacher: {
    id: string;
    name: string;
...
const TrimesterAwards: React.FC = () => {
  const me = useUser();
  ...
  const { students, awards } = data || { students: [], awards: [] };
  // add awards to students
  students.forEach((student: Student) => {
    student.awards = awards.filter(
      (award: TrimesterAward) => award.student.id === student.id,
    );
  });
  // get all awards for current trimester
  const currentAwards = awards.filter(
    // todo: change this to currentTrimester
    (award: TrimesterAward) =>
      award.trimester === currentTrimester.toString() || true,
  );
  const myAwards = awards.filter(
    (award: TrimesterAward) => award.teacher.id === me.id,
  );
...
export default TrimesterAwards;
