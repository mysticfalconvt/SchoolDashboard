...
interface Student {
  ...
  studentPbisCards: StudentPbisCard[];
  taTeacher?: TaTeacher;
}

const StudentsOfInterestPBIS: React.FC = () => {
  const me = useUser();
  ...
  const studentsWithTaTeacher =
    data?.students?.filter(
      (student: Student) =>
        student.taTeacher && student.taTeacher.id !== ADMIN_ID,
    ) || [];
    ...
    <div className="flex flex-col items-center justify-center w-full h-full">
      ...
      <div>
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              setNumberOfStudentsToDisplay(Number(e.target.value))
            }
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full h-full mt-4">
        <div className="bottom flex flex-col items-center">
          ...
          {bottomStudents.map((student: Student) => {
            const lastCardDate = student?.studentPbisCards?.[0]?.dateGiven
              ? new Date(student.studentPbisCards[0].dateGiven)
              : null;
            const date = lastCardDate
              ? lastCardDate.toLocaleDateString()
              : 'No cards';
            const daysSinceLastCard = lastCardDate
                ? Math.round(
                    (new Date().getTime() - lastCardDate.getTime()) /
                      (1000 * 60 * 60 * 24),
                  )
                : 0,
            ...
            return (
              <div
                className="border-2 border-gray-400 rounded-md flex flex-col items-center justify-center w-full mb-2 p-2"
                key={student?.id}
              >
...
export default StudentsOfInterestPBIS;
