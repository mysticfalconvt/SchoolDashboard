...
export class SmartGraphqlClient {
  ...
  async request<T = any>(
    ...
    try {
      console.log(
        `📡 Attempting GraphQL request to primary endpoint: ${this.primaryEndpoint}`,
      );
      ...
      console.warn(`Error details:`, {
        ...
        response: primaryError.response
          ? {
              ...
              data:
                typeof primaryError.response.data === 'string'
      ...
      if (this.fallbackEndpoint !== this.primaryEndpoint) {
        try {
          ...
          console.log(
            `✅ Fallback endpoint succeeded in ${fallbackDuration}ms`,
          );
          return result;
        } catch (fallbackError) {
          console.error(
            `❌ Fallback endpoint also failed: ${this.fallbackEndpoint}`,
          );
        ...
        console.error(
          `💥 No fallback available, primary and fallback endpoints are the same`,
        );
        throw primaryError;
      }
    }
  ...
  private async testConnectivity(endpoint: string): Promise<void> {
    try {
      ...
      console.log(
        `✅ Base connectivity test: ${response.status} ${response.statusText}`,
      );
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.log(
            `📊 GraphQL endpoint response: ${gqlResponse.status} ${gqlResponse.statusText}`,
          );
          const responseText = await gqlResponse.text();
          console.log(
            `📄 GraphQL response preview: ${responseText.substring(0, 200)}...`,
          );
        } catch (gqlError) {
