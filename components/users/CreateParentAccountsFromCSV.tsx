...
export default function CreateParentAccountsFromCSV() {
  ...
  const [createParentMutate, { loading: creatingParent, error, mutateAsync: createParentAsync }] = useGqlMutation(
  ...
  const processCSV = async () => {
    ...
    try {
      ...
      for (const row of csvRows) {
        ...
        if (!student) {
          console.log(`❌ No match found for "${row.First_Name} ${row.Last_Name}"`);
          console.log('Available students:', studentsData.students.map(s => ({ name: s.name, email: s.email })));
        } else {
          console.log(`✅ Matched "${row.First_Name} ${row.Last_Name}" to student:`, { name: student.name, email: student.email });
        }
        
        const result = createProcessingResult(row.First_Name, row.Last_Name, student);
        ...
        if (validateContactInfo(row['Contact 1'], row['Contact 1 Email'])) {
          ...
          const existingParent1 = checkParentExists(row['Contact 1 Email'], student.id, parentsData.parents);
          if (existingParent1) {
            ...
            try {
              ...
              if (errorMessage.includes('Unique constraint failed') && errorMessage.includes('email')) {
                result.errors.push(`Contact 1 email already exists: ${row['Contact 1 Email']}`);
              } else {
                result.errors.push(`Failed to create Contact 1: ${errorMessage}`);
              }
            }
          }
        }
        ...
        if (validateContactInfo(row['Contact 2'], row['Contact 2 Email'])) {
          ...
          const existingParent2 = checkParentExists(row['Contact 2 Email'], student.id, parentsData.parents);
          if (existingParent2) {
            ...
            try {
              ...
              const errorMessage = err?.message || err?.toString() || 'Unknown error';
              if (errorMessage.includes('Unique constraint failed') && errorMessage.includes('email')) {
                result.errors.push(`Contact 2 email already exists: ${row['Contact 2 Email']}`);
              } else {
                result.errors.push(`Failed to create Contact 2: ${errorMessage}`);
              }
            }
          }
        }
        ...
        <>
          ...
          <div className="fixed z-50 left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-11/12 max-w-xl h-auto rounded-3xl bg-gradient-to-tr from-[var(--red)] to-[var(--blue)] overflow-hidden border-2 border-[var(--blue)] shadow-2xl">
            ...
            <div className="p-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="mb-4">
                <label htmlFor="csvFile" className="block text-white font-semibold mb-2">
                  Select CSV File
                </label>
                <input
                  type="file"
        ...
        <div className="mt-4 bg-white rounded-xl p-4 shadow">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {processingResults.map((result, index) => (
              <div key={index} className={`border rounded p-3 ${result.errors.length > 0 ? 'border-red-300 bg-red-50' : 'border-gray-300'}`}>
                ...
                {result.contact1Name && (
                  <div className="text-sm mt-1">
                    <strong>Contact 1:</strong> {result.contact1Name} ({result.contact1Email})
                    {result.contact1Created && <span className="text-green-600 ml-2 font-semibold">✓ Created</span>}
                    {result.contact1Existed && <span className="text-blue-600 ml-2 font-semibold">Already Exists</span>}
                  </div>
                )}
                
                {result.contact2Name && (
                  <div className="text-sm mt-1">
                    <strong>Contact 2:</strong> {result.contact2Name} ({result.contact2Email})
                    {result.contact2Created && <span className="text-green-600 ml-2 font-semibold">✓ Created</span>}
                    {result.contact2Existed && <span className="text-blue-600 ml-2 font-semibold">Already Exists</span>}
                  </div>
                )}
                
                {result.errors.length > 0 && (
                  <div className="bg-red-100 border border-red-300 rounded p-2 mt-2">
                    <div className="text-red-800 font-semibold text-sm">⚠ Errors:</div>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div>Parent accounts already existed: {stats.parentsExisted}</div>
                    {stats.totalErrors > 0 && (
                      <div className="text-red-600">Total errors: {stats.totalErrors}</div>
                    )}
                  </>
                );
              })()}
