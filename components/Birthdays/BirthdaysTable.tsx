...
const BirthdaysTable: React.FC<BirthdaysTableProps> = ({ birthdays }) => {
    ...
    () => [
      {
        ...
        columns: [
          ...
          },
          {
            ...
            Cell: ({ cell }: { cell: any }) => (
              <DeliveredCake cake={cell.row.original} />
            ),
          },
        ],
      },
  ...
  const typesOfCakes: CakeTypeCount =
    birthdays?.reduce((acc, birthday) => {
      const { cakeType } = birthday;
      if (!acc[cakeType || 'null']) {
        acc[cakeType || 'null'] = 1;
      } else {
        acc[cakeType || 'null'] += 1;
      }
  ...
  const typesOfUndeliverdCakes: CakeTypeCount =
  ...
  const cakesLeftToDeliver = birthdays?.filter((birthday) => {
  ...
  const cakesLeftToDeliverSorted = cakesLeftToDeliver?.sort((a, b) => {
    const aDate = new Date(a.date);
    const bDate = new Date(b.date);
    const aDateWithoutYear = new Date(
      aDate.getMonth() + 1 + '/' + aDate.getDate(),
    );
    const bDateWithoutYear = new Date(
      bDate.getMonth() + 1 + '/' + bDate.getDate(),
    );
    return aDateWithoutYear.getTime() - bDateWithoutYear.getTime();
  });
